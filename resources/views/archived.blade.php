...
<body class="font-[Story] m-0 p-0">
    ...
    <div class="min-h-full" x-data="{ showButtons: true, showModal: false }">
        <nav id="navbar" class="fixed bg-stone-400 top-0 z-50 w-full transition-all duration-300">
            <div class="mx-auto max-w-7xl px-1 sm:px-2 lg:px-3">
                <div class="flex items-center justify-between h-20">
                    <a>
                    </a>
                    <div class="hidden md:flex items-center space-x-6">
                        {{-- <x-nav-link href="#home" :active="request()->is('home')">{{ __('home.nav_home') }}</x-nav-link>
                        <x-nav-link href="#ice" :active="request()->is('ice')">{{ __('home.nav_ice') }}</x-nav-link>
                        <x-nav-link href="#bar" :active="request()->is('bar')">{{ __('home.nav_bar') }}</x-nav-link> --}}
                        <x-nav-link href="/" :active="request()->is('location')">{{ __('home.home') }}</x-nav-link>
                        <x-nav-link
                            @click="
                                    showModal = true
                            "
                            class="cta-button bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition">
                            {{ __('home.offer') }}
                        </x-nav-link>
            ...
            <div class="md:hidden hidden absolute left-0 w-full top-0 bg-black bg-opacity-80 shadow-lg z-40"
                ...
                <div class="px-4 py-3 divide-y divide-gray-700 space-y-0" x-data="{ langOpen: false }">
                    <a href="/"
                        class="block py-4 px-3 text-base font-medium text-white hover:bg-gray-700">{{ __('home.home') }}</a>
                    <a @click="
        ...
        <div x-show="showModal" x-cloak class="fixed inset-0 z-[9999] flex items-center justify-center"
            ...
            <!-- The modal content -->
            <div class="relative z-10 overflow-y-auto rounded-lg w-full max-w-6xl p-12">
    ...
    <div
        class="bg-hero bg-scroll md:bg-fixed md:bg-cover bg-center bg-repeat md:bg-no-repeat [@media(min-width:1080px)]:px-32">
        <section id="events">
            <div class="w-full pt-20 flex items-center justify-center bg-stone-700 text-gray-800 font-bold">
                <div class="w-full">
                    <div>
                        <div class="w-full px-4 lg:px-20 py-10">
                            <div x-data="galleryComponent({{ json_encode(array_map(fn($f) => asset('plagaty/' . basename($f)), File::files(public_path('plagaty')))) }})"
                                class="mx-auto flex flex-wrap justify-center gap-8 max-w-screen-xl mt-2 lg:mt-6 z-30">
                                ...
                                <!-- Gallery Modal (moved outside flex items) -->
                                <div x-show="isOpen" x-cloak
                                    ...
                                    <button class="absolute top-4 right-6 text-white text-4xl"
                                        @click="closeGallery">&times;</button>
